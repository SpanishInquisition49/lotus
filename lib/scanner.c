...
void scan_token(Scanner *scanner, char c) {
    switch(c) {
        ...
        case '.':
            ...
            break;
        case '-':
            add_token(scanner, match(scanner, '>') ? SINGLE_ARROW : MINUS, NULL);
            break;
        case '+':
        ...
        case '!':
            ...
            break;
        case '=':
            add_token(scanner, match(scanner, '=') ? EQUAL_EQUAL : match(scanner, '>') ? DOUBLE_ARROW : EQUAL, NULL);
            break;
        case '<':
...
void number(Scanner *s) {
    ...
    else if(peek(s) == '.' && !is_digit(peek_next(s))) {
        s->errors[ERROR]++;
...
int keyword_get(char* text){
    ...
    if(strcmp(text, K_SUPER) == 0) return SUPER;
    if(strcmp(text, K_THIS) == 0) return THIS;
    if(strcmp(text, K_TRUE) == 0) return TRUE;
    if(strcmp(text, K_VAR) == 0) return VAR;
    if(strcmp(text, K_WHILE) == 0) return WHILE;
